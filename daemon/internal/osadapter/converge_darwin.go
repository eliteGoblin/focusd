...
func ConvergeSingleInstance(
	...
	for _, m := range convergeModes(keepMode, os.Geteuid()) {
		...
		// f0593fb (HF1 fix): SweepOrphanWorkdirs now takes the containment root as
		// an EXPLICIT param (never recomputed from mode.SupportRoot, which for Test
		// would resolve to the REAL ~/Library). Pass this domain's root.
		_, _ = SweepOrphanWorkdirs(root, keepDaemonHome)
		...
		if n, rerr := reapForeignPlatforms(root, exemptPID, exemptPath, listPlatformProcs, resolvePlatformExecs, sig.VerifyFile, killProc); rerr == nil {
			reaped += n
		}
	}
