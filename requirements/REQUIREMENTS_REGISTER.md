...
| **`daemon status` health snapshot** | Read-only health read (engine/version/per-protection recency/blocklist+pf sizes/skill files/verdict) that NEVER leaks disguised tokens — closes the "indirect question whose answer is a bypass recipe" path | ✅ shipped (FEATURE 9; redaction structural per ADR-0011) | Status is a read, not a protection; admin-only lines are "unknown" without sudo; age buckets coarse by design |
