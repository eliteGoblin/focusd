...
- **(unfiled) FEATURE 12 — out-of-band watchdog** is **defining** (ADR-0016 accepted). It closes the total-atomic-teardown gap via a cron rail running a second copy of the daemon binary. Awaiting human gate before build. Open design questions for the architect: disguised location of the watchdog copy (must survive a workdir wipe), cron-entry disguise, mutual re-install thrash avoidance, two-copy self-update sync, and whether the watchdog also verifies the platform.
